...
CREATE PROCEDURE v_definicion_integridad_Referencial2
AS
SET NOCOUNT ON;
...
SET @longitud = (select count(*) from ( 
	select COlUMN_NAME, COUNT(COLUMN_NAME) as duplicados from (
		...
		where  TABLE_NAME NOT IN (select distinct TABLE_NAME from INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
			where ORDINAL_POSITION > 1 ))T1 where CONSTRAINT_NAME LIKE 'PK%' or CONSTRAINT_NAME LIKE 'UPKCL%' group by COLUMN_NAME)T2)
		...
		Begin 
			...
			WHILE @j < @anomalias
				BEGIN
					...
					select st.parent_id, st.name, ssm.definition from sys.triggers st 
...
EXEC v_definicion_integridad_Referencial2
go
...
select COlUMN_NAME, COUNT(COLUMN_NAME) as duplicados,ROW_NUMBER() OVER (ORDER BY (select 1)) AS a from (
select TABLE_NAME,COLUMN_NAME, CONSTRAINT_NAME, ORDINAL_POSITION from INFORMATION_SCHEMA.KEY_COLUMN_USAGE 
where  TABLE_NAME NOT IN (select distinct TABLE_NAME from INFORMATION_SCHEMA.KEY_COLUMN_USAGE where ORDINAL_POSITION > 1 )
...
select TABLE_NAME, CONSTRAINT_NAME from INFORMATION_SCHEMA.KEY_COLUMN_USAGE where COLUMN_NAME = @Nombre_columna)T1
			join sys.objects so  on (T1.TABLE_NAME = so.name)
			where CONSTRAINT_NAME LIKE 'PK%' or CONSTRAINT_NAME LIKE 'UPKCL%'